    ...
    <div class="container card">
        ...
        <?php foreach ( $educations as $i => $education ) { ?>
            <div>
                ...
                <?php if ( $education->list )        { ?><ul role="list"><?php foreach ( $education->list as $li ) { ?><li><?php echo $li ?></li><?php } ?></ul><?php } ?>
            </div>
            <div><?php if ( $i < count( $educations ) - 1 ) { ?><hr><?php } else { ?><br><?php } ?></div>
        <?php } ?>
    </div>
<?php } ?>
