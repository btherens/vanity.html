...
class UInterface extends DomDocument
{
    ...
    public function _createTable( array $headers, bool $invisiblecolumn = false ): UIelement
    {
        ...
        if ( $invisiblecolumn ) { $tbl->_setClass( 'invisible-column' ); }
    ...
    ): UIelement
    {
        ...
        /* test if vartypestr is an object, and if it is an object, check for datetime type and apply varTypestr */
        if ( $varTypestr == 'object' ) { if (get_class( $value ) == 'DateTime' ) { $varTypestr = 'DateTime'; } };
        /* switch upon the variable type */
        switch ($varTypestr)
        {
    ...
    ): UIelement
    {
        ...
        if ( $class ) { $element->_setClass( $class ); }
...
class UIelement extends DomElement
{
    ...
    private $_readonly = [ '_mainAttribute' ];
    /* magic method returns private properties in the _readonly property list */
    ...
    public function setMain( &$arg1 ) { $this->_mainAttribute = &$arg1; }

    /* set an html attribute to the "main" html element (in a label:form uielement, main attributes apply to the form element) */
    public function setMainAttribute( $arg1, $arg2 = null )
    {
        /* set key value as key if no value was provided */
    ...
    public function setIdValueEvent( UIelement $elmt )
    {
        ...
        /* add js event handler to call js function upon focus out */
        if ( $id ) { $this->setMainAttribute( 'onfocusout', "setIdValue(this,'{$id}');" ); }
    }
}
